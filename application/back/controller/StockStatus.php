...
class StockStatus extends Controller
{
    ...
    public function set($id=null)
    {
        ...
        elseif ($request->isPost()) {
            ...
            $data = $request->post();
            // 数据校验是否通过
            $validate = Loader::validate('StockStatusSetValidate');// use think\Loader;
            // 批量验证传入的数据
    ...
    public function index()
    {
        ...
        // 判断是否传递了过滤条件
        // 设置
        $query = StockStatusModel::where($where);
    ...
    {
        // 利用模型完成删除即可
        StockStatusModel::destroy(input('selected/a', []));
        // 重定向
        $this->redirect('index');
    }
}