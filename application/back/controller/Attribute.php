...
class Attribute extends Controller
{
    ...
    public function set($id=null)
    {
        ...
        if ($request->isGet()) {
            ...
            return $this->fetch('set', [
                ...
                'data' => $data,
                'type_list' => TypeModel::order('sort')->select(),
            ]);
        }
        ...
        elseif ($request->isPost()) {
            ...
            $data = $request->post();
            // 数据校验是否通过
            $validate = Loader::validate('AttributeSetValidate');// use think\Loader;
            // 批量验证传入的数据
    ...
    public function index()
    {
        ...
        if ($field !== '') {
            ...
            // 分配到模板, 用于确定URL
            $order = ['field'=>'a.'.$field, 'type'=>$type];
        }

        // 关联到type获取分组名
        $query
            ->field('a.*, t.title as type')
            ->join('__TYPE__ t', 't.id=a.type_id')
            ;
    ...
    {
        // 利用模型完成删除即可
        AttributeModel::destroy(input('selected/a', []));
        // 重定向
        $this->redirect('index');
    }
}