 ...
 */
class LengthUnit extends Controller
{
    /**
     * @return mixed
     * 设置 = 添加和更新
    ...
    public function set($id=null)
    {
        ...
        if (is_null($id)) {
            $model = new LengthUnitModel();
        } else {
            $model = LengthUnitModel::get($id);
        }
        ...
        elseif ($request->isPost()) {
            ...
            $data = $request->post();
            // 数据校验是否通过
            $validate = Loader::validate('LengthUnitSetValidate');// use think\Loader;
            // 批量验证传入的数据
    ...
    public function index()
    {
        ...
        // 判断是否传递了过滤条件
        // 设置
        $query = LengthUnitModel::where($where);
    ...
    {
        // 利用模型完成删除即可
        LengthUnitModel::destroy(input('selected/a', []));
        // 重定向
        $this->redirect('index');
    }
}